...
export class RequestService {
  ...
  private static mapParameters(requestParameters?: RequestOptions) {
    ...
    const mappedParameters: {
      [index: string]: string | boolean | number | FilterOptions
    } = {}

    if (requestParameters) {
      for (const parameterKey in requestParameters) {
        let parameterValue =
          requestParameters[parameterKey as keyof RequestOptions]
        if (parameterValue) {
          const mappedOption =
            parameterKey in map ? map[parameterKey] : parameterKey
          if (parameterValue instanceof Array) {
            ...
            for (const filterKey in parameterValue) {
              const filterValue =
                parameterValue[filterKey as keyof FilterOptions]
              if (filterValue) {
  ...
  private async request<T>(
    ...
    try {
      ...
      const totalResults = headers['total']
